...
// ── Design tokens ──────────────────────────────────────────────────────────
const C = {
  bg:"#F5F0FA", card:"#FFFFFF", green:"#6B21A8", greenLight:"#A855F7",
  greenPale:"#EDE9FE", teal:"#4C1D95", accent:"#E76F51", accentLight:"#FFF0EB",
  text:"#1A1A2E", muted:"#8E9BAE", border:"#E9D5FF", softGreen:"#F3E8FF",
  purple:"#7C3AED", purplePale:"#EDE9FE", yellow:"#F4A261", yellowPale:"#FEF3E2",
};
...
const MSG_STYLE = {
  praise:{bg:"#EDE9FE",border:C.greenLight,icon:"👏",label:"Reconocimiento",labelColor:C.green},
  alert: {bg:"#FFF4F0",border:C.accent,    icon:"⚠️",label:"Aviso",         labelColor:C.accent},
  goal:  {bg:"#EDE9FE",border:C.purple,    icon:"🎯",label:"Meta semanal",  labelColor:C.purple},
...
const typeColor = t => ({"Embarazada":"#E0BBE4","Nodriza":"#FFC8A2","Adulto":"#C4B5FD","Adulto Mayor":"#C7CEEA","Bebé (mamá)":"#FFDAC1","Otro":"#DDD6FE"}[t]||C.greenPale);
...
const Card = ({children,style={}}) => (
  <div style={{background:C.card,borderRadius:18,border:`1px solid ${C.border}`,padding:18,boxShadow:"0 2px 12px rgba(108,33,168,0.06)",...style}}>{children}</div>
);
...
const Landing = ({onNutri, onPatient}) => (
  <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#F5F0FA 0%,#EDE9FE 100%)`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:32,padding:24}}>
    <div style={{textAlign:"center"}}>
      <Logo height={120} style={{marginBottom:8}}/>
      <p style={{fontSize:14,color:C.muted,marginTop:6}}>Seguimiento nutricional personalizado</p>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:14,width:"100%",maxWidth:420}}>
      {[
        ...
        <div key={c.title} onClick={c.onClick}
          style={{background:c.bg,borderRadius:20,padding:"22px 24px",cursor:"pointer",display:"flex",gap:18,alignItems:"center",boxShadow:"0 8px 24px rgba(108,33,168,0.22)",transition:"transform 0.2s"}}
...
const NutriLogin = ({onLogin, onBack}) => {
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#F5F0FA 0%,#EDE9FE 100%)`,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
...
const PatientApp = ({patient, setPatient, onLogout}) => {
  ...
  return (
    <div style={{minHeight:"100vh",background:`linear-gradient(160deg,#F5F0FA 0%,#EDE9FE 100%)`,display:"flex",justifyContent:"center",alignItems:"flex-start",padding:"0 0 0 0"}}>
      <div style={{width:"100%",maxWidth:430,background:C.bg,minHeight:"100vh",position:"relative",display:"flex",flexDirection:"column"}}>

        {/* Top bar */}
        <div style={{background:`linear-gradient(135deg,${C.teal},${C.green})`,padding:"10px 20px 10px",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
          <Logo height={36}/>
          <button onClick={onLogout} style={{background:"rgba(255,255,255,0.15)",border:"none",borderRadius:8,padding:"5px 12px",color:"rgba(255,255,255,0.85)",fontFamily:F.body,fontSize:12,cursor:"pointer"}}>Salir</button>
        </div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"20px 18px 90px"}}>
            ...
            <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:16}}>
                    ...
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
                      {ENERGIES.map(e=>(
                        <button key={e.v} onClick={()=>setEnergy(e.v)}
                          style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,background:energy===e.v?C.softGreen:"none",border:`1.5px solid ${energy===e.v?C.greenLight:C.border}`,borderRadius:12,padding:"8px 4px",cursor:"pointer",flex:1,margin:"0 2px",transition:"all 0.2s"}}>
                          <span style={{fontSize:22}}>{e.e}</span>
                          <span style={{fontSize:9,color:energy===e.v?C.green:C.muted,fontWeight:energy===e.v?700:400}}>{e.l}</span>
                        </button>
                      ))}
                    </div>
                    <Btn onClick={saveMood} disabled={!mood||!energy} full>Guardar estado del día</Btn>
                  </>
                ) : (
                  <div style={{textAlign:"center",padding:"12px 0"}}>
                    <p style={{fontSize:28,marginBottom:6}}>{MOODS.find(m=>m.v===mood)?.e} {ENERGIES.find(e=>e.v===energy)?.e}</p>
                    <p style={{fontSize:13,color:C.green,fontWeight:600}}>¡Estado registrado! 💜</p>
                  </div>
                )}
              </Card>
              ...
              <Card>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <p style={{fontFamily:F.display,fontSize:15,color:C.teal,fontWeight:600}}>Adherencia hoy</p>
                  <span style={{fontFamily:F.display,fontSize:22,fontWeight:700,color:pct>70?C.green:C.accent}}>{pct}%</span>
                </div>
                <ProgressBar value={pct} color={pct>70?C.greenLight:C.accent} height={10}/>
                <p style={{fontSize:12,color:C.muted,marginTop:8}}>{done} de {total} comidas completadas</p>
              </Card>

              {/* Messages preview */}
              {unread>0&&(
                <div style={{background:`linear-gradient(135deg,${C.teal},${C.green})`,borderRadius:16,padding:"14px 18px",display:"flex",alignItems:"center",gap:12,cursor:"pointer"}} onClick={()=>setTab("messages")}>
                  <span style={{fontSize:24}}>💌</span>
                  <div>
                    <p style={{color:"#fff",fontWeight:700,fontSize:14}}>{unread} mensaje{unread>1?"s":""} nuevo{unread>1?"s":""}</p>
                    <p style={{color:"rgba(255,255,255,0.7)",fontSize:12}}>Tu nutricionista te escribió</p>
                  </div>
              ...
              {/* Next control */}
              <Card style={{background:`linear-gradient(135deg,${C.purplePale},${C.softGreen})`,border:`1px solid ${C.greenPale}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <p style={{fontSize:12,color:C.muted,marginBottom:4}}>Próximo control</p>
                    <p style={{fontFamily:F.display,fontSize:16,color:C.teal,fontWeight:700}}>{patient.nextControl}</p>
                  </div>
                  <div style={{background:C.green,borderRadius:12,padding:"8px 14px",textAlign:"center"}}>
                    <p style={{fontFamily:F.display,fontSize:22,color:"#fff",fontWeight:700,lineHeight:1}}>{days}</p>
                    <p style={{color:"rgba(255,255,255,0.7)",fontSize:10}}>días</p>
                  </div>
                </div>
              </Card>
            </div>
          ...
          {tab==="plan" && (
            <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:12}}>
              <p style={{fontFamily:F.display,fontSize:20,color:C.teal,fontWeight:700,marginBottom:4}}>Mi plan alimentario</p>
              {patient.plan.map(meal=>(
                <div key={meal.id} onClick={()=>toggleMeal(meal.id)}
                  style={{background:meal.done?`linear-gradient(135deg,${C.softGreen},${C.greenPale})`:C.card,borderRadius:16,padding:"14px 16px",border:`1.5px solid ${meal.done?C.greenLight:C.border}`,cursor:"pointer",display:"flex",gap:14,alignItems:"center",transition:"all 0.25s",boxShadow:meal.done?"0 2px 12px rgba(168,85,247,0.12)":"none"}}>
                  <span style={{fontSize:26,flexShrink:0}}>{meal.icon}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <p style={{fontWeight:700,fontSize:14,color:meal.done?C.green:C.text}}>{meal.label}</p>
                    <p style={{fontSize:12,color:C.muted,marginTop:2,lineHeight:1.4}}>{meal.note}</p>
                  </div>
                  <div style={{width:26,height:26,borderRadius:"50%",background:meal.done?C.green:C.border,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.25s"}}>
                    {meal.done&&<span style={{color:"#fff",fontSize:13}}>✓</span>}
                  </div>
                </div>
              ))}
            </div>
          ...
          {tab==="messages" && (
            <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:12}}>
              <p style={{fontFamily:F.display,fontSize:20,color:C.teal,fontWeight:700,marginBottom:4}}>Mensajes</p>
              {patient.messages.length===0&&(
                <Card style={{textAlign:"center",padding:32}}>
                  <p style={{fontSize:32,marginBottom:8}}>💌</p>
                  <p style={{fontSize:14,color:C.muted}}>No tienes mensajes aún</p>
                </Card>
              )}
              {patient.messages.map(msg=>{
                ...
                return(
                  <div key={msg.id} onClick={()=>markRead(msg.id)}
                    style={{background:s.bg,borderRadius:16,padding:16,borderLeft:`4px solid ${s.border}`,opacity:msg.read?0.75:1,transition:"opacity 0.3s"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <span style={{fontSize:16}}>{s.icon}</span>
                        <span style={{fontSize:11,fontWeight:700,color:s.labelColor}}>{s.label}</span>
                      </div>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        <span style={{fontSize:11,color:C.muted}}>{msg.date}</span>
                        {!msg.read&&<div style={{width:8,height:8,borderRadius:"50%",background:C.green}}/>}
                      </div>
                    </div>
                    <p style={{fontSize:14,color:C.text,lineHeight:1.5}}>{msg.text}</p>
                  </div>
                );
              })}
            </div>
          ...
          {tab==="doubts" && (
            <div className="fade-up" style={{display:"flex",flexDirection:"column",gap:12}}>
              <p style={{fontFamily:F.display,fontSize:20,color:C.teal,fontWeight:700,marginBottom:4}}>Mis dudas</p>
              <Card>
...
const NutritionistPanel = ({patients, setPatients, onLogout}) => {
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:C.bg}}>
      {/* Sidebar */}
      <div style={{width:navOpen?210:62,background:`linear-gradient(180deg,${C.teal} 0%,${C.green} 100%)`,display:"flex",flexDirection:"column",padding:navOpen?"22px 14px":"22px 10px",flexShrink:0,transition:"width 0.25s ease",overflow:"hidden",position:"sticky",top:0,height:"100vh"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:navOpen?"space-between":"center",marginBottom:28}}>
          {navOpen&&<Logo height={36} style={{filter:"brightness(0) invert(1)"}}/>}
          ...
          {NUTRI_NAV.map(n=>{
            ...
            return(
              <button key={n.id} onClick={()=>{setView(n.id);setEditingId(null);}} title={!navOpen?n.label:""}
                style={{display:"flex",alignItems:"center",gap:navOpen?10:0,justifyContent:navOpen?"flex-start":"center",padding:"10px",borderRadius:12,border:"none",background:active?"rgba(255,255,255,0.18)":"none",color:active?"#fff":"rgba(255,255,255,0.65)",fontFamily:F.body,fontSize:13,fontWeight:active?700:400,cursor:"pointer",whiteSpace:"nowrap",transition:"all 0.2s"}}>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.15)",paddingTop:14,display:"flex",alignItems:"center",gap:navOpen?10:0,justifyContent:navOpen?"flex-start":"center"}}>
